...
function selStrat(el, strat) {
  ...
  if (!checkStrategyAllowed(strat)) {
    showUpgradeModal(
      `The ${strat.toUpperCase()} strategy requires a Pro plan. Free plan includes RSI and Bollinger Band strategies only.`,
      "strategy"
    );
    return;
  }
  ...
  const paramMap = {
    rsi: `<div class="fg"><label class="fl">RSI Period</label><input type="number" class="fi" id="rsi-p" value="14" min="2" max="50"></div>
    ...
    bb: `<div class="fg"><label class="fl">BB Period</label><input type="number" class="fi" id="bb-p" value="20"></div>
         <div class="fg"><label class="fl">Std Dev Multiplier</label><input type="number" class="fi" id="bb-s" value="2" step="0.1"></div>`,
    macd: `<div class="fg"><label class="fl">Fast EMA</label><input type="number" class="fi" id="mf" value="12"></div>
    ...
    ema: `<div class="fg"><label class="fl">Fast EMA</label><input type="number" class="fi" id="ef" value="9"></div>
...
function runBT() {
  ...
  // Gate: candle count
  const requestedPts = parseInt(document.getElementById('bt-pts').value) || 1000;
  const allowedPts = checkBtCandlesAllowed(requestedPts);
  if (allowedPts < requestedPts) {
    document.getElementById('bt-pts').value = allowedPts;
    showToast(`Free plan limited to ${allowedPts} candles. Upgrade to Pro for 5,000.`, 'amber');
  }

  // Gate: strategy
  if (!checkStrategyAllowed(currentStrat)) {
    showUpgradeModal(`The ${currentStrat.toUpperCase()} strategy requires a Pro plan.`, "strategy");
    return;
  }
  ...
  setTimeout(() => {
    ...
    document.getElementById('bt-info').innerHTML = `
      <div class="alert ${res.pf >= 1.2 ? 'al-g' : res.pf >= 1 ? 'al-am' : 'al-r'}" style="font-size:10px;">
        ${res.pf >= 1.2 ? '✓ Positive edge. Consider live testing with small stakes.' :
          res.pf >= 1 ? '⚠ Marginal edge. Optimise parameters before going live.' :
          '✕ Negative edge. Do not trade this live.'}
      </div>
      ${!isPro() ? `<div style="margin-top:8px;font-size:10px;color:var(--t3);text-align:center;">Free plan: ${pts} candles max. <a href="#" onclick="showUpgradeModal('Upgrade for 5,000 candles and all strategies.','bt');return false;" style="color:var(--g);">Upgrade for 5,000 ↑</a></div>` : ''}
    `;
  }, 1400);
}
...
function renderBTResults(r) {
  ...
  document.getElementById('bt-res').innerHTML = `
    <div class="m-grid">
      <div class="mc"><div class="mc-l">Win Rate</div><div class="mc-v" style="color:${wc}">${r.winRate.toFixed(1)}%</div><div class="mc-s">${r.wins}W / ${r.losses}L</div></div>
      <div class="mc"><div class="mc-l">Profit Factor</div><div class="mc-v" style="color:var(--bl)">${r.pf}</div><div class="mc-s">Gross W / Gross L</div></div>
      <div class="mc"><div class="mc-l">Max Drawdown</div><div class="mc-v" style="color:var(--r)">${r.mdd.toFixed(2)}%</div><div class="mc-s">Peak to trough</div></div>
      <div class="mc"><div class="mc-l">Expectancy</div><div class="mc-v" style="color:${r.exp >= 0 ? 'var(--g)' : 'var(--r)'}">${r.exp >= 0 ? '+' : ''}$${r.exp.toFixed(2)}</div><div class="mc-s">Avg P&L per trade</div></div>
    </div>
    <hr class="hr">
    ...
    <div style="margin:4px 0 10px;"><div class="pb2"><div class="pf2" style="width:${r.winRate}%;background:${wc}"></div></div></div>
    ...
    <hr class="hr">
    <div class="sec-lbl">Win / Loss Split</div>
    <div style="display:flex;gap:4px;margin-top:6px;">
      <div style="flex:${r.wins || 1};background:rgba(0,229,176,.2);height:8px;border-radius:4px 0 0 4px;"></div>
      <div style="flex:${r.losses || 1};background:rgba(255,61,107,.2);height:8px;border-radius:0 4px 4px 0;"></div>
    </div>
...
function renderEqChart(res, prices, cap) {
  ...
  eqChart = new Chart(document.getElementById('eq-ch').getContext('2d'), {
    type: 'line',
    ...
    options: {
      ...
      plugins: { legend: { display: false }, tooltip: { backgroundColor: '#111a24', borderColor: '#1c2d3e', borderWidth: 1 } },
      scales: { x: { display: false }, y: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#4d6e88', font: { family: 'JetBrains Mono', size: 10 }, callback: v => '$' + v.toFixed(0) } } }
    }
  });
  ...
  ddChart = new Chart(document.getElementById('dd-ch').getContext('2d'), {
    ...
    options: {
      ...
      scales: { x: { display: false }, y: { grid: { color: 'rgba(255,255,255,.04)' }, ticks: { color: '#4d6e88', font: { family: 'JetBrains Mono', size: 9 }, callback: v => v.toFixed(1) + '%' } } }
    }
  });
}
...
function renderTradePills(trades) {
  ...
  container.innerHTML = sample.map(t =>
    `<div style="width:10px;height:10px;border-radius:2px;background:${t.won ? 'rgba(0,229,176,.7)' : 'rgba(255,61,107,.7)'};" title="${t.won ? 'Win +$' + t.pnl.toFixed(2) : 'Loss -$' + Math.abs(t.pnl).toFixed(2)}"></div>`
...
function resetBT() {
  ...
  document.getElementById('bt-trade-pills').innerHTML = '';
  document.getElementById('bt-info').innerHTML = '<div class="alert al-am" style="font-size:10px;">&#9432; Simulates up to 5,000 candles of synthetic price data matching Deriv volatility indices.</div>';
}

function showToast(msg, type) {
  const t = document.createElement('div');
  const color = type === 'amber' ? '#ffb800' : type === 'red' ? '#ff3d6b' : '#00e5b0';
  t.style.cssText = `position:fixed;bottom:24px;right:24px;z-index:9000;background:#0c1219;border:1px solid ${color}44;border-radius:8px;padding:12px 16px;font-family:'Space Grotesk',sans-serif;font-size:12px;color:${color};font-weight:600;max-width:300px;`;
  t.textContent = msg;
  document.body.appendChild(t);
  setTimeout(() => { if (t.parentNode) t.remove(); }, 4000);
}

// Apply lock icons to Pro-only strategies on load
function applyStrategyLocks() {
  const lim = getPlanLimits();
  ['macd', 'ema'].forEach(s => {
    if (!lim.strategies.includes(s)) {
      const el = document.getElementById('so-' + s);
      if (el && !el.querySelector('.lock-icon')) {
        const lock = document.createElement('span');
        lock.className = 'lock-icon';
        lock.style.cssText = 'font-size:10px;color:var(--am);margin-left:4px;';
...
// Apply candle lock on select
function applyCancleLock() {
  const lim = getPlanLimits();
  const sel = document.getElementById('bt-pts');
  if (!sel) return;
  Array.from(sel.options).forEach(opt => {
    if (parseInt(opt.value) > lim.btCandles) {
