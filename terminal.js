/* =============================================
   ...
   Plans: demo → free → pro → enterprise
   ============================================= */
...
/* ── Called after any login method ── */
function onConnect() {
  const state = AppState.get();
  const plan  = getCurrentPlan();
  S.auth = true;
  S.bal  = state.bal || 10000;
  ...
  // Show demo watermark on chart
  if (plan === 'demo') showDemoWatermark();
}

function showDemoWatermark() {
  const wrap = document.getElementById("main-ch-wrap");
  if (!wrap || document.getElementById("demo-watermark")) return;
  const wm = document.createElement("div");
  wm.id = "demo-watermark";
  wm.style.cssText = "position:absolute;bottom:10px;right:14px;font-family:'JetBrains Mono',monospace;font-size:11px;color:rgba(167,139,250,.4);font-weight:700;pointer-events:none;z-index:2;letter-spacing:2px;";
  wm.textContent = "DEMO — SIMULATED";
  wrap.appendChild(wm);
}
...
function applyMartingaleGate() {
  ...
  const tog = document.getElementById("mart-tg");
...
function updStakePlaceholder() {
  const lim   = getPlanLimits();
  const input = document.getElementById("stake");
  if (!input) return;
  input.min = lim.minStake || 0.35;
  input.max = lim.maxStake;
  // Set default stake to plan minimum for free users
  if (isFree() && parseFloat(input.value) < lim.minStake) {
    input.value = lim.minStake;
    S.stake = lim.minStake;
  }
  input.title = `Min: $${lim.minStake || 0.35} — Max: $${lim.maxStake}`;
}
...
function initMainChart() {
  ...
  grad.addColorStop(0, "rgba(0,229,176,.12)");
...
function onMart() {
  ...
  if (!lim.martingale) {
    ...
    showUpgradeModal("Martingale mode requires a Pro plan.", "martingale");
    return;
  }
...
function onAssetChange() {
  ...
  if (!lim.assets.includes(sel.value)) {
    showUpgradeModal("This asset requires a Pro plan. Free plan is limited to Volatility 50 (R_50).", "asset");
...
function updRisk() {
  ...
  const tooHigh = S.stake > lim.maxStake;
  const tooLow  = !lim.isDemo && S.stake < (lim.minStake || 0);
  stakeEl.style.borderColor = (tooHigh || tooLow) ? "var(--r)" : "";
  stakeEl.title = `Min: $${lim.minStake || 0.35} — Max: $${lim.maxStake}`;
...
function execTrade() {
  ...
  if (S.stake > lim.maxStake) {
    showUpgradeModal(`${PLANS[getCurrentPlan()].name} max stake is $${lim.maxStake}. Upgrade for higher limits.`, "stake");
    return;
  }
  if (!lim.isDemo && S.stake < lim.minStake) {
    alert(`Minimum stake on the Free plan is $${lim.minStake}. Please increase your stake.`);
    return;
  }

  // Throttle checks (skip for demo)
  if (!lim.isDemo) {
  ...
  // Execution delay (free plan only)
...
function doExec() {
  ...
  const t = { id: ++S.cnt, asset: S.asset, type: S.ot, stake: S.stake, entry: S.price, openTime: Date.now(), durMs, demo: lim.isDemo };
  S.openTrades.push(t);
  S.bal -= S.stake;
  if (!lim.isDemo) { incrementDailyTrades(); setLastTradeTime(); }
...
function showCooldownToast(ms) {
  ...
  const iv = setInterval(() => {
    rem--;
    if (t.parentNode) t.textContent = `⏱ Cooldown: ${rem}s — Upgrade to Pro for instant execution`;
    if (rem <= 0) { clearInterval(iv); if (t.parentNode) t.remove(); }
  }, 1000);
}
...
function resolveExpired() {
  ...
  S.openTrades = S.openTrades.filter(t => {
    if (now - t.openTime < t.durMs) return true;
    resolveTrade(t); return false;
  });
  renderOpenPos();
}
...
function resolveTrade(t) {
  ...
  S.closed.push(t);
  S.bal = Math.max(0, S.bal + t.stake + pnl);

  // Persist (mark demo trades so they don't pollute real stats)
  const state = AppState.get();
  state.bal = S.bal;
  state.closedTrades = S.closed;
...
function closeAll() {
  S.openTrades.forEach(t => resolveTrade(t));
  S.openTrades = []; renderOpenPos();
}
...
function renderOpenPos() {
  ...
  if (!S.openTrades.length) {
    p.innerHTML = '<div style="color:var(--t3);font-size:11px;text-align:center;padding:20px 0;">No open positions</div>';
    return;
  }
  p.innerHTML = S.openTrades.map(t => `
    <div id="pos-${t.id}" style="background:var(--bg2);border:1px solid var(--border);border-radius:5px;padding:7px 10px;margin-bottom:5px;">
      <div style="display:flex;justify-content:space-between;">
        <span style="font-size:11px;font-weight:700;color:${t.type==='rise'||t.type==='call'?'var(--g)':'var(--r)'}">${t.type.toUpperCase()}${t.demo?' (DEMO)':''}</span>
...
function addLogRow(t) {
  ...
  row.className = "log-rw";
  row.innerHTML = `
    <div class="lc" style="color:var(--t2)">${ts.toTimeString().slice(0,8)}</div>
    <div class="lc">${t.asset}</div>
    <div class="lc" style="color:${t.type==='rise'||t.type==='call'?'var(--g)':'var(--r)'}">${t.type.toUpperCase()}</div>
    <div class="lc">$${t.stake.toFixed(2)}${t.demo?' <span style="font-size:9px;color:var(--pu)">demo</span>':''}</div>
    <div class="lc"><span class="badge ${t.won?'bg':'br'}" style="font-size:10px;">${t.won?'WON':'LOST'}</span></div>`;
...
function updSessStats() {
  ...
  we.textContent = wr + "%";
  we.style.color = parseFloat(wr) >= 50 ? "var(--g)" : "var(--r)";
  const pe = document.getElementById("ss-pnl");
  pe.textContent = (pnl>=0?"+$":"-$") + Math.abs(pnl).toFixed(2);
  pe.style.color = pnl >= 0 ? "var(--g)" : "var(--r)";
}

/* ── INIT — Demo starts immediately, no auth needed ── */
window.addEventListener("load", () => {
  // Always start in demo by default — no barrier
  connectDemo();
});
